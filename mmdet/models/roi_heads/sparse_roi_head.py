...
class SparseRoIHead(CascadeRoIHead):
    ...
    def forward_train(
        ...
        for stage in range(self.num_stages):
            ...
            for i in range(num_imgs):
                normalize_bbox_ccwh = bbox_xyxy_to_cxcywh(
                    proposal_list[i] / imgs_whwh[i]
                )
    ...
    def simple_test(
        self, x, proposal_boxes, proposal_features, img_metas, imgs_whwh, rescale=False
    ):
        ...
        if self.with_bbox:
            for stage in range(self.num_stages):
                ...
                bbox_results = self._bbox_forward(
                    stage, x, rois, object_feats, img_metas
                )
