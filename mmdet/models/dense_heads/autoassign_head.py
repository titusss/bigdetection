...
class CenterPrior(nn.Module):
    ...
    def __init__(
        self, force_topk=False, topk=9, num_classes=1223, strides=(8, 16, 32, 64, 128)
    ):
...
class AutoAssignHead(FCOSHead):
    ...
    def _get_points_single(self, featmap_size, stride, dtype, device, flatten=False):
        ...
        y, x = super(FCOSHead, self)._get_points_single(
            featmap_size, stride, dtype, device
        )
    ...
    def get_pos_loss_single(
        self, cls_score, objectness, reg_loss, gt_labels, center_prior_weights
    ):
    ...
    def get_neg_loss_single(
        self, cls_score, objectness, gt_labels, ious, inside_gt_bbox_mask
    ):
    ...
    def get_targets(self, points, gt_bboxes_list):
        ...
        concat_lvl_bbox_targets = [torch.cat(item, dim=0) for item in bbox_targets_list]
